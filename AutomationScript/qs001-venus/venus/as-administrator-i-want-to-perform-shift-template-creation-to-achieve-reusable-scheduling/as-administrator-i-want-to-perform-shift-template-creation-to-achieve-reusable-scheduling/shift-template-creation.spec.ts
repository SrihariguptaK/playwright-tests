...
test.describe('Shift Template Creation', () => {
  ...
  test('Validate successful shift template creation', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="template-list"]')).toContainText('Morning Shift Template');
    await expect(page.locator('[data-testid="template-list"]')).toContainText('09:00');
    await expect(page.locator('[data-testid="template-list"]')).toContainText('17:00');
  });
  ...
  test('Ensure validation for overlapping break times', async ({ page }) => {
    ...
    await page.fill('[data-testid="template-name-input"]', 'Invalid Break Template');
    await page.fill('[data-testid="start-time-input"]', '09:00');
    await page.fill('[data-testid="end-time-input"]', '17:00');
    
    // Enter break times that overlap with shift start time
    await page.fill('[data-testid="break-start-time-input"]', '08:30');
    await page.fill('[data-testid="break-end-time-input"]', '09:30');
    ...
    await expect(page.locator('[data-testid="validation-error"]')).toContainText(/break.*overlap|invalid.*break/i);
    ...
    await expect(page.locator('[data-testid="error-message"]')).toContainText(/cannot.*submit|break.*time.*invalid|overlap/i);
    
    // Verify template was not created
    await page.goto('/shift-templates');
    await expect(page.locator('[data-testid="template-list"]')).not.toContainText('Invalid Break Template');
  });
});